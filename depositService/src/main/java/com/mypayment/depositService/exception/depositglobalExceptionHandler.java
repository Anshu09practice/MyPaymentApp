...
@ControllerAdvice
public class depositglobalExceptionHandler extends ResponseEntityExceptionHandler {
    ...
    public ResponseEntity<?> handleRetryException(retryException e, WebRequest webRequest) {
        errorResponseDTO errorResponse = new errorResponseDTO(webRequest.getDescription(false), HttpStatus.BAD_REQUEST,e.getMessage(), LocalDateTime.now());
        return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(errorResponse);
    }
